...
run_cure_ngs export-v1.3.3-example /data/output/NGS_VCF \
  >"$OUTPUT_DIR/NGS_VCF.reference-export.json"
...
test "$MAF_ROWS" -eq 25
test "$(grep -vc '^#' "$OUTPUT_DIR/NGS_VCF/VCF_ALL/test_b37.vcf")" -eq 25
test "$(grep -v '^#' "$OUTPUT_DIR/NGS_VCF/VCF_ALL_MAF/test_b37.maf" | tail -n +2 | wc -l | tr -d ' ')" -eq 25
test -s "$OUTPUT_DIR/NGS_VCF/VCF_ALL_LOG/vcf2maf_batch_log.tsv"
test -d "$OUTPUT_DIR/NGS_VCF/VCF_ALL_TMP"
...
grep -q 'NCDC_batch_vcf2maf_V.1.3.3' \
  "$OUTPUT_DIR/NGS_VCF_RUNTIME_TEST/VCF_ALL_LOG/vcf2maf_batch_summary.json"
...
grep -q $'SUCCESS\tVCF has no variants; created empty MAF header' \
  "$OUTPUT_DIR/NGS_VCF_RUNTIME_TEST/VCF_ALL_LOG/vcf2maf_batch_log.tsv"
test -s "$OUTPUT_DIR/NGS_VCF_RUNTIME_TEST/VCF_ALL_MAF/empty.grch37.maf"
test -s "$OUTPUT_DIR/NGS_VCF_RUNTIME_TEST/VCF_ALL_LOG/manifests/empty.grch37.maf.manifest.json"
...
echo "Paper/V1.3.3 folder example: $OUTPUT_DIR/NGS_VCF"
echo "Bundled example VCF: $OUTPUT_DIR/NGS_VCF/VCF_ALL/test_b37.vcf"
echo "Executed V1.3.3 runtime tree: $OUTPUT_DIR/NGS_VCF_RUNTIME_TEST"
